...
# from the original file 'base_replication_cursor_async.py'
...
class BaseReplicationCursor(ClientCursor[Row]):
    """
    ...
    def identify_system(self) -> Row:
        self.execute("IDENTIFY_SYSTEM")
        return cast(Row, self.fetchone())

    def show(self, name: str) -> Row | None:
        self.execute(sql.SQL("SHOW {name}").format(name=sql.BareIdentifier(name)))
        return self.fetchone()

    def timeline_history(self, timeline: int) -> Row:
        """
    ...
    def create_replication_slot(
        ...
        * `"logical"` for logical replication cursors.
        ...
        if replication_type == ReplicationType.PHYSICAL:
            ...
            elif reserve_wal is not None and reserve_wal:
    ...
    def drop_replication_slot(self, slot_name: str, wait: bool | None = None) -> Self:
        return self.execute(
            sql.SQL(
        ...
        return sql.SQL(", ").join(
            (
                sql.SQL("{opt_name} {opt}").format(
                    opt_name=sql.SQL(opt_name), opt=str(opt)
                )
    ...
    def read_message(
        ...
        while True:
            data: Buffer = self._conn.wait_no_cancel(self._read_gen(timeout=timeout))
    ...
    def send_feedback(
        ...
        status_msg = pack_standby_status_update(
            b"r", received_lsn, flushed_lsn, applied_lsn, time_sent, request_reply
        )
        self._conn.wait_no_cancel(generators.copy_to(self._pgconn, status_msg))
    ...
    def consume_stream(
        self,
        consume: Callable[[BaseReplicationCursor[Row], ReplicationMessage], Any],
        ...
        while True:
            ...
            if not consume_all_messages or isinstance(msg, XLogDataMessage):
                ...
                if feedback_interval and (
                    ...
                    self.send_feedback(flushed_lsn=flush_lsn, applied_lsn=apply_lsn)
    ...
    def upload_manifest(self, manifest_content: Iterable[Buffer]) -> Self:
        """
        Upload a backup manifest prior to calling start_base_backup with
        the incremental option.
        """
        with self.copy("UPLOAD_MANIFEST") as copy:
    ...
    def consume_base_backup(
        self, consume: Callable[[BaseReplicationCursor[Row], BackupMessage | Row], Any]
    ) -> None:
        """
    ...
    def read_backup_message(self, timeout: float = 0.0) -> BackupMessage | Row | None:
        """
