...
from .rows import RowFactory, Row
...
class ServerCursor(ServerCursorMixin['Connection[Any]', Row], Cursor[Row]):
    ...
    @overload
    def __init__(self, connection: Connection[Row], name: str, *, scrollable: bool | None=None, withhold: bool=False):
        ...
    

    @overload
    def __init__(self, connection: Connection[Any], name: str, *, row_factory: RowFactory[Row], scrollable: bool | None=None, withhold: bool=False):
        ...
    

    def __init__(self, connection: Connection[Any], name: str, *, row_factory: RowFactory[Row] | None=None, scrollable: bool | None=None, withhold: bool=False):
        Cursor.__init__(self, connection, row_factory=row_factory or connection.row_factory)
    ...
    def execute(self, query: Query, params: Params | None=None, *, binary: bool | None=None, **kwargs: Any) -> Self:
        """
        ...
        if self._pgconn.pipeline_status:
            raise e.NotSupportedError('server-side cursors not supported in pipeline mode')
    ...
    def executemany(self, query: Query, params_seq: Iterable[Params], *, returning: bool=True) -> None:
    ...
    def __next__(self) -> Row:
        ...
        if self._iter_rows is None or self._page_pos >= len(self._iter_rows) >= self.itersize:
            with self._conn.lock:
