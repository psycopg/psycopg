...
def test_query_parse_cache_size(conn):
    ...
    tests = [(f"select 1 -- {'x' * 3500}", (), h0, m0 + 1), (f"select 1 -- {'x' * 3500}", (), h0 + 1, m0 + 1), (f"select 1 -- {'x' * 4500}", (), h0 + 1, m0 + 1), (f"select 1 -- {'x' * 4500}", (), h0 + 1, m0 + 1), (f"select 1 -- {'%s' * 40}", ('x',) * 40, h0 + 1, m0 + 2), (f"select 1 -- {'%s' * 40}", ('x',) * 40, h0 + 2, m0 + 2), (f"select 1 -- {'%s' * 60}", ('x',) * 60, h0 + 2, m0 + 2), (f"select 1 -- {'%s' * 60}", ('x',) * 60, h0 + 2, m0 + 2)]
...
@pytest.mark.parametrize('query', ['copy testcopy from stdin', 'copy testcopy to stdout'])
...
def test_binary_cursor_execute(conn):
    with raiseif(conn.cursor_factory is psycopg.ClientCursor, psycopg.NotSupportedError) as ex:
...
def test_executemany(conn, execmany):
    cur = conn.cursor()
    cur.executemany(ph(cur, 'insert into execmany(num, data) values (%s, %s)'), [(10, 'hello'), (20, 'world')])
...
def test_executemany_name(conn, execmany):
    cur = conn.cursor()
    cur.executemany(ph(cur, 'insert into execmany(num, data) values (%(num)s, %(data)s)'), [{'num': 11, 'data': 'hello', 'x': 1}, {'num': 21, 'data': 'world'}])
    cur.execute('select num, data from execmany order by 1')
    rv = cur.fetchall()
...
def test_executemany_returning(conn, execmany):
    cur = conn.cursor()
    cur.executemany(ph(cur, 'insert into execmany(num, data) values (%s, %s) returning num'), [(10, 'hello'), (20, 'world')], returning=True)
    assert cur.rowcount == 1
...
def test_executemany_returning_discard(conn, execmany):
    cur = conn.cursor()
    cur.executemany(ph(cur, 'insert into execmany(num, data) values (%s, %s) returning num'), [(10, 'hello'), (20, 'world')])
    assert cur.rowcount == 2
    with pytest.raises(psycopg.ProgrammingError):
        cur.fetchone()
    assert cur.nextset() is None
...
def test_executemany_no_result(conn, execmany):
    cur = conn.cursor()
    cur.executemany(ph(cur, 'insert into execmany(num, data) values (%s, %s)'), [(10, 'hello'), (20, 'world')], returning=True)
    assert cur.rowcount == 1
...
def test_executemany_rowcount_no_hit(conn, execmany):
    ...
    assert cur.rowcount == 0
    cur.executemany(ph(cur, 'delete from execmany where id = %s returning num'), [(-1,), (-2,)])
    assert cur.rowcount == 0


@pytest.mark.parametrize('query', ['insert into nosuchtable values (%s, %s)', 'copy (select %s, %s) to stdout', 'wat (%s, %s)'])
...
def test_executemany_null_first(conn, fmt_in):
    ...
    cur.executemany(ph(cur, f'insert into testmany values (%{fmt_in.value}, %{fmt_in.value})'), [[1, None], [3, 4]])
    with pytest.raises((psycopg.DataError, psycopg.ProgrammingError)):
        cur.executemany(ph(cur, f'insert into testmany values (%{fmt_in.value}, %{fmt_in.value})'), [[1, ''], [3, 4]])
...
@pytest.mark.parametrize('query, params, want', [('select %(x)s', {'x': 1}, (1,)), ('select %(x)s, %(y)s', {'x': 1, 'y': 2}, (1, 2)), ('select %(x)s, %(x)s', {'x': 1}, (1, 1))])
...
def test_stream(conn):
    cur = conn.cursor()
    recs = []
    for rec in cur.stream(ph(cur, "select i, '2021-01-01'::date + i from generate_series(1, %s) as i"), [2]):
        recs.append(rec)
...
def test_stream_sql(conn):
    cur = conn.cursor()
    recs = list(cur.stream(sql.SQL("select i, '2021-01-01'::date + i from generate_series(1, {}) as i").format(2)))
...
@pytest.mark.parametrize('query', ['create table test_stream_badq ()', 'copy (select 1) to stdout', 'wat?'])
...
def test_stream_error_python_to_consume(conn):
    ...
    with pytest.raises(ZeroDivisionError):
        with closing(cur.stream('select generate_series(1, 10000)')) as gen:
            for rec in gen:
                1 / 0
    assert conn.info.transaction_status in (pq.TransactionStatus.INTRANS, pq.TransactionStatus.INERROR)
...
def test_stream_binary_cursor(conn):
    with raiseif(conn.cursor_factory is psycopg.ClientCursor, psycopg.NotSupportedError):
        cur = conn.cursor(binary=True)
        recs = []
...
def test_error_no_result(conn):
    ...
    with pytest.raises(psycopg.ProgrammingError, match='last operation.*command status: SET'):
        cur.fetchone()
    
    cur.execute('')
    with pytest.raises(psycopg.ProgrammingError, match='last operation.*result status: EMPTY_QUERY'):
        cur.fetchone()
...
def test_results_after_execute(conn, count):
    with conn.cursor() as cur:
        cur.execute(';'.join((f'select * from generate_series(1, {i})' for i in range(count))))
...
def test_results_after_executemany(conn, count, returning):
    with conn.cursor() as cur:
        cur.executemany(ph(cur, 'select * from generate_series(1, %s)'), [(i,) for i in range(count)], returning=returning)
        ress = list((res.fetchall() for res in cur.results()))
        if returning:
            assert ress == [[(j + 1,) for j in range(i)] for i in range(count)]
        else:
