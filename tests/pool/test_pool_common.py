...
from ..acompat import Event, sleep, gather, is_alive, skip_async, skip_sync, spawn
...
def test_dead_client(pool_cls, dsn):
    ...
    with pool_cls(dsn, min_size=min_size(pool_cls, 2), max_size=2) as p:
        results: list[int] = []
        ts = [spawn(worker, args=(i, timeout)) for i, timeout in enumerate([0.4, 0.4, 0.1, 0.4, 0.4])]
        gather(*ts)
...
def test_cancellation_in_queue(pool_cls, dsn):
    ...
    with pool_cls(dsn, min_size=min_size(pool_cls, nconns), max_size=nconns, timeout=1) as p:
        p.wait()
