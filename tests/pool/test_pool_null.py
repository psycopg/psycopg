...
def test_generic_connection_type(dsn):
    ...
    with pool.NullConnectionPool(dsn, connection_class=MyConnection[MyRow], kwargs={'row_factory': class_row(MyRow)}, configure=configure) as p1:
...
def test_non_generic_connection_type(dsn):
    ...
    with pool.NullConnectionPool(dsn, connection_class=MyConnection, configure=configure) as p1:
        with p1.connection() as conn1:
...
def test_no_queue_timeout(proxy):
    with pool.NullConnectionPool(kwargs={'host': proxy.client_host, 'port': proxy.client_port}) as p:
...
def test_intrans_rollback(dsn, caplog):
    ...
    def worker():
        with p.connection() as conn:
            ...
            cur = conn.execute("select 1 from pg_class where relname = 'test_intrans_rollback'")
