...
from ..acompat import Event, sleep, gather, skip_sync, spawn
...
def test_generic_connection_type(dsn):
    ...
    with pool.ConnectionPool(dsn, connection_class=MyConnection[MyRow], kwargs=dict(row_factory=class_row(MyRow)), configure=configure) as p1:
...
def test_non_generic_connection_type(dsn):
    ...
    with pool.ConnectionPool(dsn, connection_class=MyConnection, configure=configure) as p1:
        with p1.connection() as conn1:
            cur1 = conn1.execute('select 1 as x')
            row1, = cur1.fetchall()
...
def test_intrans_rollback(dsn, caplog):
    ...
    with pool.ConnectionPool(dsn, min_size=1) as p:
        ...
        with p.connection() as conn2:
            ...
            cur = conn2.execute("select 1 from pg_class where relname = 'test_intrans_rollback'")
...
@pytest.mark.slow
@pytest.mark.timing
@pytest.mark.parametrize('min_size, want_times', [(2, [0.25, 0.25, 0.35, 0.45, 0.5, 0.5, 0.6, 0.7]), (0, [0.35, 0.45, 0.55, 0.6, 0.65, 0.7, 0.8, 0.85])])
...
def test_reconnect_failure(proxy, async_cb):
    ...
    with pool.ConnectionPool(proxy.client_dsn, name='this-one', min_size=1, reconnect_timeout=1.0, reconnect_failed=failed) as p:
        p.wait(2.0)
        proxy.stop()
...
def test_reconnect_after_grow_failed(proxy):
    ...
    with pool.ConnectionPool(proxy.client_dsn, min_size=4, reconnect_timeout=1.0, reconnect_failed=failed) as p:
        assert ev.wait(2.0)
...
def test_connect_check(dsn, caplog, autocommit):
    ...
    with pool.ConnectionPool(dsn, min_size=2, kwargs={'autocommit': autocommit}, check=pool.ConnectionPool.check_connection) as p:
        p.wait(1.0)
        with p.connection() as conn:
            pid1 = conn.info.backend_pid
            with p.connection() as conn2:
...
def test_getconn_check(dsn, caplog, autocommit):
    ...
    with pool.ConnectionPool(dsn, kwargs={'autocommit': autocommit}, min_size=2, check=pool.ConnectionPool.check_connection) as p:
        p.wait(1.0)
        with p.connection() as conn:
            pid1 = conn.info.backend_pid
            with p.connection() as conn2:
...
def test_connect_check_timeout(dsn, proxy):
    proxy.start()
    with pool.ConnectionPool(proxy.client_dsn, min_size=1, timeout=1.0, check=pool.ConnectionPool.check_connection) as p:
        p.wait()
...
def test_stats_check(dsn):
    with pool.ConnectionPool(dsn, min_size=1, check=pool.ConnectionPool.check_connection) as p:
        p.wait()
        with p.connection() as conn:
            pid = conn.info.backend_pid
...
def test_override_close(dsn):
    with pool.ConnectionPool(dsn, connection_class=ReturningConnection, min_size=2) as p:
        p.wait()
...
def test_close_returns_custom_class(dsn):
    ...
    with pool.ConnectionPool(dsn, min_size=2, connection_class=MyConnection, close_returns=True) as p:
        p.wait()
        conn = p.getconn()
        assert not conn.closed
        assert len(p._pool) == 1
...
def test_close_returns_no_loop(dsn):
    with pool.ConnectionPool(dsn, min_size=1, close_returns=True, max_lifetime=0.05) as p:
        conn = p.getconn()
        sleep(0.1)
        assert len(p._pool) == 0
...
def test_override_close_no_loop_subclass(dsn):
    with pool.ConnectionPool(dsn, min_size=1, max_lifetime=0.05, connection_class=ReturningConnection) as p:
        conn = p.getconn()
        sleep(0.1)
        assert len(p._pool) == 0
        sleep(0.1)  # wait for the connection to expire
...
def test_get_config_rotates_connections(dsn):
    ...
    p = pool.ConnectionPool(conninfo=rotating_config, kwargs=rotating_kwargs, min_size=2, max_lifetime=0.2, open=False)
...
def test_get_config_raise_exception(dsn, caplog):
    ...
    p = pool.ConnectionPool(conninfo=failing_conninfo, kwargs=failing_kwargs, min_size=1, max_lifetime=0.1, open=False, reconnect_timeout=1.0)
    ...
    reconnection_warnings = [rec for rec in caplog.records if 'reconnection attempt' in rec.message.lower()]
