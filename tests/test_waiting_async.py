...
if True:  # ASYNC
    ...
    waitfns = [
        ...
        pytest.param(
            "wait_epoll_async", marks=pytest.mark.skipif("not hasattr(select, 'epoll')")
        ),
        pytest.param(
...
@skip_sync
def test_async_wait_name(waitfn):
    # these are autogenerated, so check their __name__
    name = waitfn
    waitfn = getattr(waiting, waitfn)
    assert waitfn.__name__ == name
