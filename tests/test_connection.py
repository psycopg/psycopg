...
from .acompat import sleep, skip_async, skip_sync
...
def test_context_inerror_rollback_no_clobber(conn_cls, conn, dsn, caplog):
    ...
    with pytest.raises(ZeroDivisionError):
        with conn_cls.connect(dsn) as conn2:
            ...
            conn.execute('select pg_terminate_backend(%s::int)', [conn2.pgconn.backend_pid])
            1 / 0
...
@pytest.mark.parametrize('args, kwargs, want', [((), {}, ''), (('',), {}, ''), (('host=foo.com user=bar',), {}, 'host=foo.com user=bar hostaddr=1.1.1.1'), (('host=foo.com',), {'user': 'baz'}, 'host=foo.com user=baz hostaddr=1.1.1.1'), (('dbname=foo port=5433',), {'dbname': 'qux', 'user': 'joe'}, 'dbname=qux user=joe port=5433'), (('host=foo.com',), {'user': None}, 'host=foo.com hostaddr=1.1.1.1')])
def test_connect_args(conn_cls, monkeypatch, setpgenv, pgconn, fake_resolve, args, kwargs, want):
    got_conninfo: str
...
@pytest.mark.parametrize('args, kwargs, exctype', [(('host=foo', 'host=bar'), {}, TypeError), (('', ''), {}, TypeError), ((), {'nosuchparam': 42}, psycopg.ProgrammingError)])
...
def test_transaction_param_default(conn, param):
    ...
    cur = conn.execute('select current_setting(%s), current_setting(%s)', [f'transaction_{param.guc}', f'default_transaction_{param.guc}'])
...
def test_set_transaction_param_reset(conn, param):
    conn.execute('select set_config(%s, %s, false)', [f'default_transaction_{param.guc}', param.non_default])
    conn.commit()
...
def test_cancel_safe_error(conn_cls, proxy, caplog):
    ...
    with conn_cls.connect(proxy.client_dsn) as conn:
        proxy.stop()
        with pytest.raises(e.OperationalError, match='(Connection refused)|(connect\\(\\) failed)') as ex:
