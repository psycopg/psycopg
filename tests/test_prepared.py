...
@pytest.mark.parametrize('query', ['create table test_no_prepare ()', pytest.param("notify foo, 'bar'", marks=pytest.mark.crdb_skip('notify')), 'set timezone = utc', 'select num from prepared_test', 'insert into prepared_test (num) values (1)', 'update prepared_test set num = num * 2', 'delete from prepared_test where num > 10'])
...
def test_params_types(conn):
    conn.execute('select %s, %s, %s', [dt.date(2020, 12, 10), 42, Decimal(42)], prepare=True)
    stmts = get_prepared_statements(conn)
...
def test_change_type_execute(conn):
    ...
    for i in range(3):
        ...
        conn.cursor().execute('INSERT INTO preptable (bar) VALUES (%(enum_col)s::prepenum[])', {'enum_col': ['foo']})
        conn.rollback()
...
def test_change_type_executemany(conn):
    for i in range(3):
        conn.execute("CREATE TYPE prepenum AS ENUM ('foo', 'bar', 'baz')")
        conn.execute('CREATE TABLE preptable(id integer, bar prepenum[])')
        conn.cursor().executemany('INSERT INTO preptable (bar) VALUES (%(enum_col)s::prepenum[])', [{'enum_col': ['foo']}, {'enum_col': ['foo', 'bar']}])
        conn.rollback()
