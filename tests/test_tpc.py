...
class TestTPC:
    ...
    def test_xid_encoding(self, conn, tpc):
        ...
        cur = conn.cursor()
        cur.execute('select gid from pg_prepared_xacts where database = %s', (conn.info.dbname,))
        assert '42_Z3RyaWQ=_YnF1YWw=' == cur.fetchone()[0]
    

    @pytest.mark.parametrize('fid, gtrid, bqual', [(0, '', ''), (42, 'gtrid', 'bqual'), (2147483647, 'x' * 64, 'y' * 64)])
