...
from cpython.list cimport PyList_New, PyList_SET_ITEM, PyList_GET_SIZE, PyList_Append, PyList_AsTuple
...
cdef class RecordLoader(_CRecursiveLoader):
    ...
    cdef object cload(self, const char *data, size_t length):
        ...
        record = tuple(_parse_text_record(data+1, length-2, &(self.scratch), &(self.sclen)))
...
cdef list _parse_text_record(const char *data, size_t length, char **scratch, size_t *sclen):
    cdef char *buf = data
    cdef list record = []
...
cdef object _parse_record_token(char **bufptr, const char *bufend, char **scratch, size_t *sclen):
    ...
    while end < bufend:
        if (end[0] == b',' or end[0] == b')') and not quoted:
            break
        elif end +1 < bufend and ((end[0] == b'\\' and end[1] == b'\\') or (end[0] == b'"' and end[1] == b'"')):
            num_escapes += 1
            end += 1
        elif end[0] == b'"':
            quoted = False
    ...
    while src < end:
        if src + 1 < end and ((src[0] == b'\\' and src[1] == b'\\') or (src[0] == b'"' and src[1] == b'"')):
            dst[0] = src[1]
            src += 2
        else:
            dst[0] = src[0]
